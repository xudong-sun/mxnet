 ...
 * \file proposal_mask_target_sv.cc
 * \brief C++ version proposal target
 * \author Tian Li, Yuntao Chen, Zehao Huang, Pengfei Chen
 */
#include <algorithm>
#include <cstdio>
#include "./proposal_mask_target_sv-inl.h"
...
template<>
Operator *CreateOp<cpu>(ProposalMaskTargetSVParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ProposalMaskTargetSVOp<cpu, DType>(param);
  })
  return op;
}

template<>
Operator *CreateOp<gpu>(ProposalMaskTargetSVParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ProposalMaskTargetSVOp<gpu, DType>(param);
  })
  return op;
}

Operator *ProposalMaskTargetSVProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(ProposalMaskTargetSVParam);

MXNET_REGISTER_OP_PROPERTY(ProposalMaskTargetSV, ProposalMaskTargetSVProp)
.describe("C++ version proposal target")
.add_argument("rois", "Symbol", "rois")
.add_argument("gt_boxes", "Symbol", "gtboxes")
.add_arguments(ProposalMaskTargetSVParam::__FIELDS__());
