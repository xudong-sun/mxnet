...
const std::chrono::steady_clock::time_point reference_time = std::chrono::steady_clock::now();
...
__global__ void nonlinear_clip_kernel(
  DType* boxes,       // B, N, 4
...
template <typename DType>
__global__ void _fore_back_enhance_kernel(
  const DType* scores, // B * N * C
...
template <typename DType>
inline void PostDetctionForward(const Tensor<gpu, 2, DType> &rois,
                                const Tensor<gpu, 3, DType> &scores,
                                const Tensor<gpu, 3, DType> &bbox_deltas,
                                const Tensor<gpu, 2, DType> &im_info,
                                const Tensor<gpu, 3, DType> &batch_boxes,
                                const Tensor<gpu, 2, DType> &batch_boxes_rois,
                                const mxnet::op::PostDetectionOpParam &params_ ) {
  // cudaProfilerStart();
  tick();
  DType* out_batch_boxes = new DType[batch_boxes.shape_.Size()]; // batch_boxes.dptr_;
  DType* out_batch_boxes_rois = new DType[batch_boxes_rois.shape_.Size()]; // batch_boxes_rois.dptr_;
  ...
  DType* _pred_boxes_cu; CUDA_SAFE_CALL(cudaMalloc( &_pred_boxes_cu, N*B*(4*C)*sizeof(DType)));

  std::vector<DType> cpu_im_info(3);
  CUDA_SAFE_CALL(cudaMemcpy(cpu_im_info.data(),
                            im_info.dptr_,
                            sizeof(DType) * cpu_im_info.size(),
  ...
  CUDA_SAFE_CALL(cudaMemcpy(_pred_boxes, _pred_boxes_cu,
              sizeof(DType) * bbox_deltas.shape_.Size(),
              cudaMemcpyDeviceToHost));
  DType* _enhance_scores = new DType[N*B*C];
  DType* _enhance_scores_cu; CUDA_SAFE_CALL(cudaMalloc(&_enhance_scores_cu, N*B*C*sizeof(DType)));
  cuda::_fore_back_enhance_kernel<<<B, N>>>(scores.dptr_, _enhance_scores_cu, C);
  CUDA_SAFE_CALL(cudaMemcpy( _enhance_scores, _enhance_scores_cu,
              sizeof(DType) * N*B*C,
              cudaMemcpyDeviceToHost));
  int* _keep = new int[N];
  ...
  for(int b=0; b<B; b++) {
    ...
    int keep_num = cuda::weighted_nms(
      _boxes_batch, _score_batch, _class_batch, _order_batch, n_box_this_batch,
      params_.nms_thresh_lo, params_.nms_thresh_hi, _keep_out);
    ...
    CUDA_SAFE_CALL(cudaMemcpy(batch_boxes_rois.dptr_, out_batch_boxes_rois,
      sizeof(DType)*batch_boxes_rois.shape_.Size(), cudaMemcpyHostToDevice));
...
template<>
Operator *CreateOp<gpu>(PostDetectionOpParam param, int dtype) {
  // std::cout << "CreateOp<gpu>" << std::endl;
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new PostDetectionOp<gpu, DType>(param);
  })
  return op;
}
