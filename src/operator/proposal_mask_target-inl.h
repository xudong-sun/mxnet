...
struct ProposalMaskTargetParam : public dmlc::Parameter<ProposalMaskTargetParam> {
  ...
  DMLC_DECLARE_PARAMETER(ProposalMaskTargetParam) {
    ...
    DMLC_DECLARE_FIELD(poly_len).describe("Length of encoded polygons reprensenting mask of each ROI");
...
class ProposalMaskTargetOp : public Operator {
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    const index_t num_image         = in_data[proposal_mask_target_enum::kRois].shape_[0];
    ...
    const index_t _batch_rois = param_.img_rois * param_.batch_images;
    ...
    if (param_.ohem) {
        ...
        for (index_t i = 0; i < num_image; ++i) {
          ...
          TensorContainer<cpu, 2, DType> kept_gtpolys_i(Shape2(kept_gtbboxes_size_i, param_.poly_len), 0.f);
...
class ProposalMaskTargetProp : public OperatorProperty {
 public:
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    index_t _batch_rois = param_.img_rois * param_.batch_images;
    ...
    auto mask_target_shape = Shape4((index_t)(_batch_rois * param_.fg_fraction), param_.num_classes, param_.mask_size, param_.mask_size);
